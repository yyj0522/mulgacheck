...
import { Trash2, Lock, RefreshCw, BarChart3, LogOut } from "lucide-react";
import { loginAction, checkAdmin, deleteCommentAction, logoutAction } from "./actions"; // 서버 액션 불러오기
...
export default function AdminPage() {
  ...
  const [initChecking, setInitChecking] = useState(true); // 초기 권한 확인 중
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = await loginAction(password); // 서버에 비밀번호 확인 요청
    if (res.success) {
      setIsAuthenticated(true);
      fetchComments();
    } else {
  ...
  const deleteComment = async (id: string) => {
    ...
    const res = await deleteCommentAction(id); // 서버에 삭제 요청
    if (res.success) {
      alert("삭제되었습니다.");
      fetchComments(); // 목록 갱신
    } else {
