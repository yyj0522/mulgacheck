...
구글의 Gemini 2.5 Flash와 Google Search Grounding(검색 연동) 기술을 결합하여, 사용자가 입력한 예산(원화)을 실시간 현지 환율로 변환하고 현재 영업 중인 실제 장소들의 물가를 반영한 100% 현실적인 여행 일정을 단 10초 만에 생성합니다.
...
### 1. 빈약한 콘텐츠(Thin Content)로 인한 SEO 색인 누락 문제 해결
* **문제 상황:** 배포 후 20일이 지나도록 핵심 페이지(`/plan`, `/budget`)와 메인 페이지가 구글 서치 콘솔에서 '발견됨 - 현재 색인이 생성되지 않음' 상태에 머물러 노출수가 0이었습니다. 입력창과 버튼만 존재하는 툴(Tool) 형태의 페이지를 구글 봇이 빈약한 콘텐츠로 판단했으며, `www` 유무에 따른 표준 URL 혼란까지 겹친 상황이었습니다.
* **해결 방안:** 
  1. `layout.tsx`에 `metadataBase`와 `alternates: { canonical: "/" }`를 강제 지정하여 구글 봇의 중복 문서 혼란을 해결했습니다.
  2. 사용자가 일정을 생성하고 자랑하기(공유)를 누를 때마다 `/community/[id]` 형태의 SSR 페이지가 자동 생성되도록 **프로그래매틱 SEO 아키텍처**를 구축했습니다. `generateMetadata`를 통해 페이지마다 고유의 키워드가 삽입되게 하고, `sitemap.ts`에서 DB를 조회해 수백 개의 일정 URL을 동적으로 뿜어내어 구글에 자동으로 색인되도록 문제를 타개했습니다.

### 2. AI 예산 오차 및 환율 미적용 문제 극복
...
### 3. 무분별한 API 호출 방지를 위한 Rate Limit 및 에러 핸들링
